...
def calculate_metrics(options_chain, current_price):
    ...
    options_chain['remaining_business_days'] = options_chain['expiry'].apply(
        lambda expiry: np.busday_count(
            datetime.now().date(),
            pd.to_datetime(expiry).date() + timedelta(days=1),
            weekmask='1111100'
        )
    )
    ...
    options_chain['calendar_days'] = options_chain['expiry'].apply(
        lambda x: max((datetime.strptime(x, '%Y-%m-%d').date() - today).days, 1)
    )
    
    # 使用日历天数计算年化收益率
    options_chain['annualized_return'] = (
        options_chain['lastPrice'] / options_chain['strike'] * (365 / options_chain['calendar_days']) * 100
    )
...
def format_output(filtered_df):
    display_columns = [
        ...
        'impliedVolatility', 'delta', 'annualized_return', 'dte', 'expiry'
    ]
